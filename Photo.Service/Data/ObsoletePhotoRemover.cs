...
namespace PhotoBank.Photo.Service.Data
{
    public class ObsoletePhotoRemover
    {
        ...
        public void Remove()
        {
            var allPhotosFromDatabase = _repositoryFactory.Get<IPhotoRepository>().GetAllPhotos().ToHashSet();
            var allPhotosFromDisk = Directory.GetFiles(PhotoSettings.PhotoDatabasePath).ToList();
